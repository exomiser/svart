...
public final class VariantKeys {
    ...
    private static final long BASE_MASK = 0b11;
    ...
    public record VariantKey(long key, String chrom, int pos, String ref, String alt) implements Comparable<VariantKey> {
        ...
        public static VariantKey of(int chrom, int pos, String ref, String alt) {
            long key = VariantKeys.encodeVariantKey(chrom, pos, ref, alt);
            return new VariantKey(key, CHROMS[chrom], pos, ref, alt);
        }
    ...
    /**
     * Decodes the chromosome from a variant key to its numeric representation.
     *
     * @param key the variant key to decode
     * @return numeric chromosome code (0-25)
     */
    public static int decodeChromToInt(long key) {
        int chrBits = (int) ((key & VKMASK_CHROM) >>> VKSHIFT_CHROM);
        return (chrBits >= 1 && chrBits <= 25) ? chrBits : 0;
    }
    ...
    public static String decodeRef(long variantKey) {
        int refLength = refLength(variantKey);
        return decodeAllele(refLength, 21, variantKey);
    }
    ...
    public static String decodeAlt(long variantKey) {
        int altLength = altLength(variantKey);
        return decodeAllele(altLength, 21 - (refLength(variantKey) << 1), variantKey);
    }

    private static String decodeAllele(int alleleLength, int alleleOffset, long variantKey) {
        if (alleleLength == 0) {
            return "";
        } else if (alleleLength == 1) {
            return decodeBaseAsString((int) ((variantKey >>> alleleOffset) & BASE_MASK));
        } else if (alleleLength >= 2 && alleleLength <= 10) {
            return newAlleleString(alleleLength, alleleOffset, variantKey);
        } else {
            return "?";
        }
    }

    private static String newAlleleString(int alleleLength, long alleleOffset, long variantKey) {
        byte[] bases = new byte[alleleLength];
        for (int i = 0; i < alleleLength; i++) {
            bases[i] = decodeBase((byte) ((variantKey >>> (alleleOffset - ((long) i << 1))) & BASE_MASK));
        }
    ...
    private static String decodeBaseAsString(int base) {
    ...
    private static byte decodeBase(int base) {
